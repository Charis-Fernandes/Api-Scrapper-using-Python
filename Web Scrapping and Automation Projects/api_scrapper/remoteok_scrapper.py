#API Scrapper that puts the data from this api URL- https://datausa.io/api/data?drilldowns=Nation&measures=Population 
#and puts it in excel sheet and can mail


import requests
import xlwt
from xlwt import Workbook
import smtplib
from os.path import basename
from email.mime.application import MIMEApplication
from email.mime.multipart import MIMEMultipart
from email.mime.text import MIMEText
from email.utils import COMMASPACE, formatdate

BASE_URL= 'https://datausa.io/api/data?drilldowns=Nation&measures=Population'
USER_AGENT= 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36 OPR/109.0.0.0'
REQUEST_HEADER={
    'User-Agent': USER_AGENT,
    'Accept-Language': 'en-US, en;q=0.5',
}

def get_public_data():
    res = requests.get(url=BASE_URL, headers= REQUEST_HEADER)
    return res.json()

def truncate_string(value, max_length=32767):                        #This step is optional
    """Truncate strings longer than max_length."""
    if isinstance(value, str) and len(value) > max_length:
        return value[:max_length]
    return value

def output_to_xls(data):
    wb = Workbook()
    job_sheet = wb.add_sheet('Data')
    headers = list(data[0].keys())
    for i in range(0, len(headers)):
        job_sheet.write(0, i, headers[i])
    for i in range(0, len(data)):
        job = data[i]
        values = [truncate_string(v) for v in job.values()]
        for x in range(0, len(values)):
            job_sheet.write(i + 1, x, values[x])
    wb.save('public_data.xls')

def send_email(send_from, send_to, subject, text, files=None):
    assert isinstance(send_to, list)
    msg = MIMEMultipart()
    msg['From'] = send_from
    msg['To'] = COMMASPACE.join(send_to)
    msg['Date'] = formatdate(localtime=True)
    msg['Subject'] = subject

    msg.attach(MIMEText(text))

    for f in files or []:
        with open(f, "rb") as fil:
            part = MIMEApplication(fil.read(), Name=basename(f))
        part['Content-Disposition'] = f'attachment; filename="{basename(f)}"'
        msg.attach(part)

    smtp = smtplib.SMTP('smtp.gmail.com: 587')
    smtp.starttls()
    smtp.login(send_from, 'your password')
    smtp.sendmail(send_from, send_to, msg.as_string())
    smtp.close()

if __name__ == "__main__":
    print("HELLO")
    response_json = get_public_data()
    if 'data' in response_json:
        json_data = response_json['data']
        output_to_xls(json_data)
        send_email('enter sender email', ['enter receiver email'],
               'US PUBLIC DATA', 'Please, find attached US public data to this email',
               files=['public_data.xls'])
    else:
        print("Unexpected JSON structure:", response_json)